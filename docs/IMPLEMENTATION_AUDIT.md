...
| Persistencia de producto multiusuario | `auth.py`, `persistence.py`, `postgres.py`, memoria namespaced y pruebas de reinicio/cross-tenant | **Real local para SQLite y PostgreSQL.** Runs, approvals, sesiones, rate limits y memoria sobreviven reinicios y se particionan por tenant. PostgreSQL aporta estado compartido; siguen faltando object storage, backup/restore operacional, failover y capacidad medida. |
