...
| Reproducible Python graph | Three `.in` files, three hash locks, locked pip-tools/build toolchain, wheel verification scripts, Docker and CI. | **Verified across Python 3.11 lock/test and Python 3.12 image runtime.** Base images/actions/SBOM/signing remain open. |
...
Verification on 21 July 2026: Oxlint passed, 33 frontend tests passed, Vite production build passed, 28 Python tests passed from a hash-locked wheel, all locks regenerated byte-identically, Helm lint/template and both safety guards passed, and the Python 3.12 packaged image passed HttpOnly session/CSRF/run/Greenlight/package/audit/metrics/revocation smoke. Docker's nested `vfs` daemon failed to register a base-image layer; the documented Buildah `vfs` + `chroot` alternative completed the identical Dockerfile and runtime smoke.
