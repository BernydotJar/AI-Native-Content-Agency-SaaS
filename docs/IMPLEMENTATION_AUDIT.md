...
Verification on 21 July 2026: Oxlint passed, 33 frontend tests passed, Vite production build passed, 33 Python tests passed from a hash-locked wheel, all locks regenerated byte-identically, Helm lint/template and both safety guards passed, and the Python 3.13.14 Alpine packaged image passed HttpOnly session/CSRF/run/Greenlight/package/audit/metrics/revocation smoke. Supply-chain verification generated SBOM, vulnerability/license evidence, provenance and two verified Cosign bundles with no registry publication. Docker's nested daemon remained unavailable; the documented Buildah `vfs` + `chroot` alternative completed the Dockerfile and runtime smoke.
