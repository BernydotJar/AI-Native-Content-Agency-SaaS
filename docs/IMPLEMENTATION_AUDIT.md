...
| Context7, browser y GitHub en runtime | Catálogos TS/Python con estados mock; registro `video-use` pinneado | **No implementado como integración.** Los nombres representan contratos de adapter; `video-use` está `reviewed_disabled` en un manifest GET-only. El producto no hace llamadas, navegación, render ni cambios remotos. |
