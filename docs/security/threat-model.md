...
| TM-018 | Routine deploy identity can change foundation IAM, SQL, services, registry policy, alerts, budget, or foundation state | Foundation and runtime use separate state roots. Plan can read required state and mutate only a disposable runtime `.tflock`; apply can mutate only the runtime-state prefix. GitHub apply has Cloud Run administration, read-only verification roles, and `actAs` on one runtime account; forbidden admin roles are rejected. | Terraform role/condition tests, safe permission probe, plan gate, and post-apply role verification. |
