...
async function login(client, key, subject) {
  ...
  requireValue(await client.eval(`(() => { const dialog=document.querySelector('[role="dialog"]'); const username=dialog?.querySelector('input[type="text"]'); const credential=dialog?.querySelector('input[type="password"]'); if(!username||!credential)return false; const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; setter.call(username,${JSON.stringify(subject)}); username.dispatchEvent(new Event('input',{bubbles:true})); username.dispatchEvent(new Event('change',{bubbles:true})); setter.call(credential,${JSON.stringify(key)}); credential.dispatchEvent(new Event('input',{bubbles:true})); credential.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`), "username or credential input missing");
  await sleep(100);
