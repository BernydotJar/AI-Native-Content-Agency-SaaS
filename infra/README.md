...
- `environments/dev/`: the separately reviewed foundation state. It creates/adopts the dev project, services, runtime identity and IAM, Artifact Registry, PostgreSQL 15 Cloud SQL, alert delivery and budget. Its apply identity uses an exact custom Cloud Run service/job role and repository-scoped image reader—not `roles/run.admin`. It is not planned or applied by the routine deployment workflow.
...
8. Configure three protected GitHub environments. `dev-build` receives only the image provider/account, `dev-plan` receives only the read-only plan provider/account, and `dev` receives the exact custom runtime deploy provider/account plus required reviewers and the short-lived exact attestation secret. Confirm that its project roles contain `projects/PROJECT_ID/roles/agencyRuntimeDeployer` and the reviewed viewer roles, while the `agency-images` repository contains only `roles/artifactregistry.reader` for that identity.
